...
| Flag | Required |
|------|----------|
| `PROACTIVE` | Either flag enables the section |
| `KAIROS` | Either flag enables the section |

The section is only included when `proactiveModule?.isProactiveActive()` returns `true`.

## Key Design Decisions

1. **Tick-based keep-alive**: Uses `<tick>` messages as heartbeats rather than continuous polling
2. **Sleep as explicit action**: Forces the model to call `Sleep` tool rather than outputting idle text
3. **Cache-aware pacing**: Prompt cache expires after 5 minutes — the model must balance sleep duration against cache reuse
4. **Focus-based autonomy**: Terminal focus state modulates how independently the agent acts
5. **Anti-narration**: Explicitly prohibits play-by-play narration of actions
